English | **[Русский](../../ru/context/roadmap.md)**
...
> **2026-06 delivery:** Phases 1–4 largely implemented (`20b8518` → `6299610`). Highlights: GHCR+SBOM+cosign, WebAuthn, Object Lock XML, LDAP sync worker, federation ListObjectsV2 proxy, Postgres HA tooling, STS session tokens (user-bound), NATS event sink, storage-class transition, nightly feature-audit CI, bundled Swagger UI. See [competitiveness roadmap](../../specs/roadmap/README.md).
...
|----|----------|------|--------|--------|-------|
| AUD-01 | Ops | Rebuild and publish `storage-server` Docker image after each release; CI build/push on `main` | M | **done** | [publish-main.yml](../../../.github/workflows/publish-main.yml) pushes `:main` and `:sha-*` on merge to `main` |
| AUD-02 | Ops | Document Docker registry proxy (`scripts/ensure-docker-pull-proxy.cmd`, `127.0.0.1:10801`) or auto-disable when proxy down | S | **done** | README §Local development; proxy wait loop in script |
...
| AUD-08 | UX | User bucket list empty state — clear 409/500 when create fails (e.g. FK error) | S | **done** | Buckets page shows list API errors; create returns 409 only for conflicts |
...
| ID | Category | Item | Effort | Status | Notes |
|----|----------|------|--------|--------|-------|
| AUD-10 | Ops | Grafana `datasafe-overview` — pre-provision Prometheus scrape on fresh install; import JSON in repo | M | **done** | Feature-audit `Grafana datasafe-overview panel query` + `datasafe_http_requests_total` Prometheus checks |
| AUD-11 | Ops | Re-enable storage-server Docker healthcheck (wget/curl on `/healthz`) | S | **done** | `docker-compose.yml` wget healthcheck |
| AUD-12 | UX | Gateway replication visibility — UI indicator when remote S3 bucket policy adjusted for public-read | S | **done** | Console banner + per-rule warning (`gateway.tsx`); feature-audit health check |
...
| AUD-16 | UX | Recursive folder delete confirmation — mirror API `object_count` on conflict in UI | S | **done** | Dialog + toast show `object_count` from 409 (`ApiError`) |
...
| AUD-19 | Testing | Versioning / object versions cycle in audit script | S | **done** | Enable → put×2 → list `/versions` → GET `?versionId=` in `feature-audit-test.ps1` |
...
| Tenant-scoped bucket names + grants | **done** | [Spec](../specs/tenant-bucket-isolation-tz.md); `(tenant_id, name)` uniqueness; tenant admin Access tab |
...
| A.5 | STS AssumeRole | **done** | `POST /api/v1/sts/assume-role`; session tokens in SigV4; credentials bound to authenticated user |
| A.6 | Event notifications | **partial** | Webhook + **NATS** (`STORAGE_NATS_URL`); Kafka/RabbitMQ planned |
| A.7 | SSE-S3 | **partial** | AES-256-GCM via `STORAGE_SSE_MASTER_KEY` |
| A.8 | Cluster health probe | **done** | Live `/healthz` every 10s; cluster metrics |
...
| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 1.1 | Bucket Versioning | **done** | S3 PUT/GET `?versioning` (Enabled/Suspended); delete markers; console Versions tab + Suspend UI (v1.2.0) |
...
| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 2.6 | Trash (Soft Delete) | **done** | `.datasafe-trash`; restore/purge; 7/30/90d auto-purge |
...
| 24 | Storage Classes | **partial** | Hot/Warm/Cold metadata; **transition API** between classes |
