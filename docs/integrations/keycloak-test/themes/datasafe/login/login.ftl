...
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled??; section>
    <#if section = "header">
        <#-- Title slot kept empty: brand is rendered in "form" so it is not
             swallowed by #kc-page-title { display:none } / parent header chrome. -->
    <#elseif section = "form">
        <div class="ds-login-brand">
            <div class="ds-login-logo-wrap" aria-hidden="true">
                <img src="${url.resourcesPath}/img/logo.svg" alt="" class="ds-login-logo" width="28" height="28"/>
            </div>
            <h1 class="ds-login-title">Датасейф S3</h1>
            <p class="ds-login-subtitle">Войдите для управления бакетами, объектами и ключами доступа.</p>
        </div>
        <div id="kc-form">
            ...
            <#if realm.password>
                <form id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                    ...
                    <div class="${properties.kcFormGroupClass!}">
                        ...
                        <div class="${properties.kcInputGroup!} ds-password-group" dir="ltr">
                            <input tabindex="3" id="password" class="${properties.kcInputClass!}" name="password" type="password" autocomplete="current-password"
                                   aria-invalid="<#if messagesPerField.existsError('username','password')>true</#if>"/>
                            <button class="${properties.kcFormPasswordVisibilityButtonClass!} ds-password-toggle" type="button" aria-label="${msg('showPassword')}"
